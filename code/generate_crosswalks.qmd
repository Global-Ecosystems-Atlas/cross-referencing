...
dat_out <- dat_long |> 
  group_by(data_id_code, band_layer_name, in_class_value) |> 
  ...
  # Create one csv file per band layer in the sub-folder 'per_band_layer_name'
  split_dir <- path(sub_dir, "per_band_layer_name")
  dir_create(split_dir)
  
  dat_out |>
    group_split(band_layer_name) |>
    walk(function(df) {

      split_filename <- glue::glue("{df$Source_ID[1]}_{df$band_layer_name[1]}_GEA_crosswalk_table.csv")
