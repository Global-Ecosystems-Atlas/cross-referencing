...
# Generate the list of EFG codes
```{r}
# Load EFG codes from Github
efg_codes <- read_csv(
  "https://raw.githubusercontent.com/Global-Ecosystems-Atlas/metadata/refs/heads/main/pixel-values/efg_pixel_values.csv") |> 
  pull(2) |> 
  unique()

# Keep only true EFG codes (.i.e remove any "0" or "NA" column)
# Filter for Letter(s) + Number + Point + Number. Regex logic:
# ^[A-Z]+   : Starts with one or more uppercase letters
# [0-9]+    : Followed by one or more digits
# \\.       : A literal period
# [0-9]+$   : Ends with one or more digits
efg_codes <- efg_codes |>
  as.character() |>
  str_subset("^[A-Z]+[0-9]+\\.[0-9]+$")

# Add "0" at the beginning and "NA" at the end.
# 0 is for true no data and NA is for non-crosswalkable data
efg_codes <- c("0", efg_codes, "NA")
...
front_cols <- c(
  ...
  "Source_ID", "data_id_code", "layer_id", "layer_name",
...
make_dataset_template <- function(df_subset) {
  ...
  band_blocks <- df_subset %>%
    mutate(
      .n_rows = map_int(n_map_classes, ~ safe_int(.x, data_id = data_id, band = layer_name))
    ) %>%
    ...
    pmap(function(Source_ID, data_id_code, license = NULL, get_alignment_firspass = NULL,
                  ...
                  documentation_score = NULL, layer_id, layer_name, year_start = NULL, 
                  year_end = NULL, n_map_classes = NULL, .n_rows, ...) {
      n <- as.integer(.n_rows)
